...
ScanCount_by_Visit_DepartmentDescription_test$CONCEPTSTORES_Return <- NULL
ScanCount_by_Visit_DepartmentDescription_test$OTHERDEPARTMENTS_Return <- NULL
...
save(fit,file="rf_v4_department.rda")
#load("rf_v4_department.rda")

#diagnose the model
a <- as.data.frame(table(ScanCount_by_Visit_DepartmentDescription$TripType))
b <- as.data.frame(importance(fit))
c<- as.data.frame(fit$err.rate)
